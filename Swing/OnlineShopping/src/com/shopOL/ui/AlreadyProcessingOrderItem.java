...
public class AlreadyProcessingOrderItem extends com.shopOL.myFrame.RightPanel {
	...
	public static void main(String[] args) {
		...
		frame.getContentPane().add(new AlreadyProcessingOrderItem(frame));
		frame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
		frame.pack();
		frame.setVisible(true);
	}

	public AlreadyProcessingOrderItem(JFrame frame) {
		super();
		initGUI(frame);
	}
	...
	private String sql2 = " status='end'";
	private String oid = "";
	private int id;
					...
					public void actionPerformed(ActionEvent arg0) {
						if (!oid.equals("")) {
							...
							JOptionPane.showMessageDialog(
									AlreadyProcessingOrderItem.this, "请先选择d订单");
						}
					}
				});
			}
	...
	void setTable(String sql) {
		...
		for (Vector<String> vector : data) {
			...
			vector.add(5, salesOrderBean.getAdminName());
		}
	...
	class ButtonAct implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (str.length() == 0) {
				...
				String sql = AlreadyProcessingOrderItem.sql;
				if (Type.isLong(str)) {
					...
					AdminBean adminBean = AdminDao.getAdminBean2(str);
					id = adminBean.getId();
					sql = sql + " or adminId=" + id;
				}
				// 执行数据库查询
				sql = sql + " and" + sql2;
				setTable(sql);
